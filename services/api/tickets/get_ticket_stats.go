...
func GetTicketStats(stats models.TicketStat) (models.TicketStat, error) {
	...
	if stats.StartDate != "" {
		...
		db.Model(&models.Ticket{}).Select("tickets.id").Joins("inner join ticket_default_types on ticket_default_types.id = tickets.ticket_default_type_id and ticket_default_types.status = ? ", "active").Where("tickets.status = ? and ticket_default_types.closure_authorizer && ? and tickets.id IN ?", "pending", "{"+stats.ClosureID+"}", ticket_id).Count(&stats.Closure)

		db.Where("id IN ? and status = ?", ticket_id, "unresolved").Model(&models.Ticket{}).Count(&stats.Unresolved)

		db.Model(&models.Ticket{}).Where("id In ? and priority = ?", ticket_id, "high").Where("status = ? or status = ?", "pending", "unresolved").Count(&stats.HighPriority)

...
func ListTicketDefault(filters models.Filter) ([]models.TicketDefault, *gorm.DB) {
	...
	db = db.Model(&models.TicketDefaultType{}).Select("ticket_default_types.id, ticket_default_types.ticket_type,ticket_default_types.status as type_status,ticket_default_timings.status as timing_status,ticket_default_timings.expiry_duration ,ticket_default_timings.tat ,ticket_default_timings.conditions,ticket_default_timings.status as timing_status ,groups.name as group_name,groups.tags, ticket_default_groups.status as group_status").Joins("left join ticket_default_groups on ticket_default_groups.ticket_type = ticket_default_types.ticket_type").Joins("left join ticket_default_timings on ticket_default_timings.ticket_type = ticket_default_types.ticket_type").Joins("left join groups on groups.id = ticket_default_groups.group_id").Scan(&ticket_default)
